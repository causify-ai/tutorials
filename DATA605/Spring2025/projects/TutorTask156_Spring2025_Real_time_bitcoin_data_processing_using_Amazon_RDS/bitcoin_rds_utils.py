...
if missing_vars:
    ...
    logger.warning("Please create a .env file with these variables or set them in your environment")
...
def get_db_connection_pool(min_connections=1, max_connections=10):
    ...
    if _connection_pool is None:
        ...
        try:
            ...
            logger.info(f"Connection pool created with {min_connections}-{max_connections} connections")
...
def aggregate_hourly_data(conn, days_to_aggregate=7):
    ...
    """
    logger.info(f"Aggregating hourly Bitcoin data for the last {days_to_aggregate} days")
    ...
    try:
        ...
        for hour in hours:
            ...
            # Add to batch
            batch_data.append((hour, open_price, high_price, low_price, close_price, avg_volume))
...
def get_bitcoin_price_history(conn, days=7, use_hourly=False):
    ...
    """
    logger.info(f"Retrieving {days} days of Bitcoin price history (hourly={use_hourly})")
...
def fetch_and_store_historical_bitcoin_data(days=30, batch_size=7):
    """
    ...
    :param batch_size: Number of days to fetch in each batch to avoid rate limits
    ...
    days_remaining = min(days, 90)  # Max 90 days for free tier
    records_inserted = 0
    
    while days_remaining > 0:
        batch_days = min(days_remaining, batch_size)
        end_date = datetime.now() - timedelta(days=days-days_remaining)
        start_date = end_date - timedelta(days=batch_days)
        
        logger.info(f"Fetching batch: {start_date.date()} to {end_date.date()} ({batch_days} days)")
        ...
        params = {
            "vs_currency": "usd",
            "days": str(batch_days),
            "interval": "hourly"  # hourly data points for better accuracy
        }
        
        try:
            # Fetch historical data
            response = requests.get(url, params=params, headers=API_CONFIG['headers'], timeout=15)
            ...
            try:
                ...
                # Insert data into database using batch approach
                for datapoint in historical_data:
                    ...
                    # If a row was inserted (not conflicted), this will return an ID
                    result = cursor.fetchone()
                    if result is not None:
                        records_inserted += 1
                
                conn.commit()
                logger.info(f"Batch inserted with {records_inserted} new records")
            ...
            # Update remaining days and add delay to avoid rate limiting
            days_remaining -= batch_days
            if days_remaining > 0:
                logger.info(f"Sleeping 2 seconds before next batch, {days_remaining} days remaining...")
                time.sleep(2)
                
        except Exception as e:
            logger.error(f"Error fetching or processing batch: {e}")
            days_remaining -= batch_days  # Skip this batch and continue
        
    logger.info(f"Historical data fetch complete. Inserted {records_inserted} new records")
...
def calculate_bollinger_bands(df: pd.DataFrame, window: int = 20, num_std: float = 2.0) -> pd.DataFrame:
    ...
    """
    logger.info(f"Calculating Bollinger Bands with {window}-period window and {num_std} std")
