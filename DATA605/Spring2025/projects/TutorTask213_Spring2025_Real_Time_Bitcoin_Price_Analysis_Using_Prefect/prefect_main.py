...
POSTGRES_URL = POSTGRES_URL = os.getenv("POSTGRES_URL")

# ✅ Email alert function
def send_email_alert(subject, body):
    sender = ALERT_EMAIL
    password = EMAIL_APP_PASSWORD
    receiver = sender  # sending to self
...
def validate_data(data):
    if not data or "price" not in data or data["price"] <= 0:
        get_run_logger().warning(" Invalid data.")
        return None
    return data
...
def visualize_price(data):
    ...
    try:
        ...
        logger.warning(f" Could not read from database for visualization: {e}")
        return
    ...
    plt.savefig("price_trend.png")  # Optional: save image to disk
    buf.seek(0)
    ...
    logger.info(" Trend chart artifact created.")
...
def detect_trend():
    ...
    msg = f" Bitcoin price change detected:\n\nCurrent: ${latest['price']}\nPrevious: ${previous['price']}\nChange: {change:.2%}"
