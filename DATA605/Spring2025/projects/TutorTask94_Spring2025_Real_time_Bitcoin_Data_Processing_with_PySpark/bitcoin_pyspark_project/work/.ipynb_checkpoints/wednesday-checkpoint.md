# Wednesday Notebook Detailed Explanation

This document explains every cell in `wednesday.ipynb`, detailing its purpose and the functions imported from the `bitcoin_utils.py` utilities file.
...
**Explanation:**
- Imports `plot_actual_vs_predicted_prices`.
- Executes the function, which:
  1. Converts the Spark predictions DataFrame to a Pandas DataFrame with proper datetime parsing.
  2. Uses Matplotlib to draw both actual and predicted price lines.
  3. Configures plot aesthetics (title, axes labels, legend, grid).
  4. Saves the figure to `output_plot.png` in the working directory.
...
**Explanation:**
This final cell is empty (no code) and can be safely removed or used for additional custom experimentation or notes.
