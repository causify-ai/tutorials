...
def convert_filtered_df_to_pandas():
    pdf = df_filtered.select("Datetime", "Close").orderBy("Datetime").toPandas()
...
def train_and_evaluate_gbt_regressor():
    ...
    # Optional: Add hour/day features for time patterns
    from pyspark.sql.functions import hour, dayofweek
...
def plot_actual_vs_predicted_prices():
    ...
    # 1. Select only what you need and convert to Pandas
    pdf = predictions.select("Datetime", "Close", "prediction").orderBy("Datetime").toPandas()
