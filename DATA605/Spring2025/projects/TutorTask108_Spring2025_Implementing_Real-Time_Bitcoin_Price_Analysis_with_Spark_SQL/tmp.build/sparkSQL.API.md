...
This tutorial uses the 'DataFrame API' due to the integration with Python.
...
Via the 'sql' function on the Spark Session, we can programmatically run SQL queries and return the results as a DataFrame. In this way, we can easily write common query statements:
1. CREATE TABLE
2. INSERT INTO TABLE
3. SELECT ... FROM ...

We can create a table and then write more complex queries by including filters, window functions, additional statements, embedded queries, CASE expression, WITH clause, joins, and much more. 
