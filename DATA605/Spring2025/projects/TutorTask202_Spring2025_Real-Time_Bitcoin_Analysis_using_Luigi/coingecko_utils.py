...
def fetch_btc_price_history(days=1, interval="minute"):
...
class FetchDataTask(luigi.Task):
    ...
    def run(self):
        df = fetch_btc_price_history(days=1)
...
class VisualizeDataTask(luigi.Task):
    ...
    def output(self):
        return luigi.LocalTarget(f"data/plot_{self.date}.png")
    def run(self):
        df = pd.read_csv(self.input().path, parse_dates=["timestamp"])
        plt.figure(figsize=(12, 4))
        plt.plot(df["timestamp"], df["price"], label="Price")
        plt.plot(df["timestamp"], df["forecast"], label="Forecast", linestyle="--")
        plt.scatter(df[df["anomaly"]]["timestamp"], df[df["anomaly"]]["price"], color="red", label="Anomaly")
        plt.title("BTC Price Forecast and Anomaly Detection")
...
class AlertTask(luigi.Task):
    ...
    def run(self):
        ...
        if not anomalies.empty:
            alert_message = f"🚨 ALERT: Anomalies detected on {self.date}:\n" + "\n".join(anomalies['timestamp'].astype(str).tolist())
            print(alert_message)
            msg = EmailMessage()
            ...
            with smtplib.SMTP_SSL("smtp.gmail.com", 465) as smtp:
                smtp.login(os.getenv("ALERT_EMAIL_FROM"), os.getenv("ALERT_EMAIL_PASSWORD"))
