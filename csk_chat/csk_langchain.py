...
class CSKChatConfig:
    ...
    def _validate(self) -> None:
        ...
        if not self.weaviate_api_key:
            raise ValueError(
                "API_KEY environment variable is required for Weaviate access"
            )
...
class WeaviateRetriever:
    ...
    with an existing Weaviate Retrieval API, extracting filename
    metadata and handling API communication.
    """
    ...
    def __init__(self, config: CSKChatConfig):
        ...
        _LOG.debug(
            "Initialized WeaviateRetriever with URL: %s", config.weaviate_url
        )
    ...
    def retrieve_documents(self, query: str) -> List[DocumentRecord]:
        ...
        try:
            ...
            _LOG.info(
                "Retrieved %d documents from API", len(retrieval_response.records)
            )
            return retrieval_response.records
        except httpx.HTTPStatusError as e:
            _LOG.error(
                "HTTP error calling retrieval API: %d - %s",
            ...
            _LOG.error("Data processing error calling retrieval API: %s", e)
            return []
            ...
            if "filepath" in record.metadata:
                ...
                if filepath:
                    ...
                    if not record.title:
                        record.title = link
                    _LOG.debug(
                        "Extracted link: %s from filepath: %s", link, filepath
                    )
    ...
    def _extract_link_from_filepath(self, filepath: str) -> str:
        ...
        github_base_url = (
            "https://github.com/causify-ai/helpers/tree/"
            "74f3d1ad9b5b453b2babbe9c870adf381b35ce45/docs"
        )
...
- When GitHub links are provided in the context (shown in parentheses),
  ALWAYS include them in your response
- Format references like: "According to [filename.md](github-link)..." or
  "As documented in [filename.md](github-link)..."
- If multiple sources are used, list all relevant links at the end of your
  response under a "References:" section
...
def _print_response(result: Dict, *, show_metadata: bool = True) -> None:
    ...
    if show_metadata and result.get("source_files"):
        ...
        print(
            f"   Source files: {', '.join(filter(None, result['source_files']))}"
        )
    print("=" * 60)
...
def _run_single_query_mode(
    chat_system: CSKChatSystem, query: str, *, show_metadata: bool = True
) -> None:
    """
...
def _test_connections(config: CSKChatConfig) -> None:
    ...
    try:
        llm = langchain_anthropic.ChatAnthropic(
            anthropic_api_key=config.anthropic_api_key, model=config.model_name
        )
        llm.invoke("test")
...
def _parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--no-metadata", action="store_true", help="Hide metadata in output"
    )
    parser.add_argument(
    ...
    try:
        ...
        if args.query:
            _run_single_query_mode(
                chat_system, args.query, show_metadata=not args.no_metadata
            )
